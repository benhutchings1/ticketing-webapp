...
# User model
class User(db.Model):
    user_id = db.Column(        db.Integer(8), primary_key=True, autoincrement=True)
...
class Event(db.Model):
    ...
    venueRelate = db.relationship("Venue")
...
class UserTicket(db.Model):
    ticket_id = db.Column(      db.Integer(8), primary_key=True, autoincrement=True)
    event_id = db.Column(       db.Integer(8), db.ForeignKey('event.event_id'), nullable=False)
    user_id = db.Column(        db.Integer(8), db.ForeignKey('user.user_id'), nullable=False)
    cipher_key = db.Column(     db.String(256), nullable=False)
    ticket_type = db.Column(    db.String(64), nullable=False)
    valid = db.Column(          db.Boolean, nullable=False)

    eventRelate = db.relationship("Event")
    userRelate = db.relationship("User")
